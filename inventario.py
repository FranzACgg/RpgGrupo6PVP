...
ITEM_KEYS_VALUES = {'id_item':0,'nombre':'','tipo':'','cantidad':0,'equipado':False}
...
def construir_cadenas_inventario(inventario, cursor, desvio_cursor):
    ...
    if len(inventario) == 0:
        cadenas.append("[dim]~ Vacío ~[/]") # [dim]...[/] es markup de rich para texto grisado.
    else:
        ...
        for i in range(desvio_cursor, rango):
            ...
            if i == cursor:
                ...
                cadenas.append(f"[dim]{selector}{nombre:<26} {cantidad:>4}{equipado}[/]")
...
def construir_detalle(inventario, cursor):
    ...
    if item['tipo'] == 'equipable':
        estado = "[bold cyan]Equipado[/]" if item['equipado'] else "[dim]No equipado[/]"
        detalle += f"\n[dim]Estado:[/] {estado}"
    return detalle
...
def renderizar_inventario(inventario, cursor, desvio_cursor, mensaje=""):
    ...
    contenido_items = construir_cadenas_inventario(inventario, cursor, desvio_cursor)
    ...
    separador = "[dim]─────────────────────────────────────────[/]"
    contenido_total = f"{contenido_items}\n{separador}\n{detalle}\n{separador}\n{controles}"
    ...
    console.print(Align(panel, align="center", vertical="middle", height=console.size.height))
    ...
    while tecla != 'q':
        ...
        if len(inventario) > 0:
            ...
            if tecla == 'w':
                ...
                if item:
                    estado = "Equipaste" if item['equipado'] else "Desequipaste"
                    mensaje = f"{estado} {item['nombre']}"
            elif tecla == 't':
                nombre_item = inventario[cursor]["nombre"]
                renderizar_inventario(inventario, cursor, desvio_cursor, f"Descartar {nombre_item}? [S/N]")
                confirmacion = ''
...
if __name__ == "__main__":
    ...
    items_prueba = [
        {'id_item': 1, 'nombre': 'Pocion de HP pequeña', 'tipo': 'consumible', 'cantidad': 5, 'equipado': False},
        {'id_item': 2, 'nombre': 'Escudo de Obsidiana', 'tipo': 'equipable', 'cantidad': 1, 'equipado': False},
        {'id_item': 3, 'nombre': 'Gema de vida', 'tipo': 'clave', 'cantidad': 2, 'equipado': False},
        {'id_item': 4, 'nombre': 'Hacha de Mitril', 'tipo': 'equipable', 'cantidad': 1, 'equipado': True},
        {'id_item': 5, 'nombre': 'Pocion de Fuerza Grande', 'tipo': 'consumible', 'cantidad': 3, 'equipado': False},
        {'id_item': 6, 'nombre': 'Pocion Alucinogena', 'tipo': 'consumible', 'cantidad': 2, 'equipado': False},
        ...
        {'id_item': 10, 'nombre': 'Casco de Dullahan', 'tipo': 'equipable', 'cantidad': 1, 'equipado': True},
        {'id_item': 11, 'nombre': 'Amuleto del Rey', 'tipo': 'clave', 'cantidad': 1, 'equipado': False},
        {'id_item': 12, 'nombre': 'Pocion de Potencia', 'tipo': 'consumible', 'cantidad': 4, 'equipado': False},
    ]
