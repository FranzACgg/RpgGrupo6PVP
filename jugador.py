...
def mover(tecla):
    ...
    df, dc = 0, 0
    if   tecla == TECLAS_MOVIMIENTO[2]: df =  1   # s
    elif tecla == TECLAS_MOVIMIENTO[0]: df = -1   # w
    elif tecla == TECLAS_MOVIMIENTO[1]: dc = -1   # a
    elif tecla == TECLAS_MOVIMIENTO[3]: dc =  1   # d
    else: return
...
def cambio_de_mapa():
    ...
    Actualiza estado["mapa_actual"] y estado["numero_mapa"].
    """
    ...
    def _cargar(nuevo_mapa, nuevo_numero, nueva_pos, nuevo_debajo):
        mapa_actual[pos_p[0]][pos_p[1]] = "O"   # restaurar entrada en mapa viejo
